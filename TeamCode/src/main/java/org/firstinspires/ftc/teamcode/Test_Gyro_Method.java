...
public class Test_Gyro_Method extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        dist1 = distanceSensorLeft.getDistance(DistanceUnit.MM)+25.4*9.75;

        angles = imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
        double angle1 = AngleUnit.DEGREES.fromUnit(angles.angleUnit, angles.firstAngle);
        ...
        angles = imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
        double angle2 = AngleUnit.DEGREES.fromUnit(angles.angleUnit, angles.firstAngle);

        double pretheta = angle2-angle1;

        telemetry.addData("pretheta" , pretheta);
        telemetry.addData("heading", heading);
        telemetry.update();
        ...
        dist2 = distanceSensorLeft.getDistance(DistanceUnit.MM)+25.4*9.75;

        double a = Math.sqrt( dist2*dist2+dist1*dist1-2*dist1*dist2*Math.cos(pretheta));

        double B = Math.asin(dist1*Math.sin(pretheta)/a);

        double distFinal = dist2*Math.sin(B);



        telemetry.addData("final thing" , distFinal/25.4);
    ...
    //Drive Methods
        ...
        public void craterArmDeploy () {
            ...
            craterArmServo.setPosition(0.4);
        }
